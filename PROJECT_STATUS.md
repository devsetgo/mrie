...
- mrie owns its own database/schema independently (`src/db_tables.py`:
  ...
  `Notifications`, `AboutPage`, `WebAuthnCredentials`) — originated as a
  port of a sibling project's (dsg) table shapes but now evolves
  independently.
- Alembic (`alembic/`) manages schema for Postgres (dev + prod) —
  `alembic upgrade head`, run out-of-band (devcontainer
  `postCreateCommand`, CI, `make alembic-migrate`, or a deploy step), never
  on app boot. SQLite (the in-memory driver `tests/conftest.py` hard-codes,
  and the real file-based driver `make run-dev-workers` uses) still calls
  `async_db.create_tables()` on boot instead (`src/resources.py`) —
  `dsg_lib`'s per-dialect `server_default` expressions mean a frozen
  autogenerated Postgres migration can't run against SQLite.
- `NoteMetrics`'s plain scalar aggregates (word_count, note_count, etc.)
  live in a `note_metrics_scalars` Postgres materialized view, computed
  directly from `notes` and refreshed by `update_notes_metrics()`; the
  `metrics` JSON blob (rolling averages, streaks, mood-weighted means, tag
...
- Local dev: `DB_DRIVER=postgres`, pointed at the devcontainer's
  ...
  whenever the driver isn't postgres and `users` is empty at boot; it never
  runs against Postgres, so a fresh Postgres dev DB starts empty until you
  register a passkey and add real notes/links. `DB_DRIVER=memory` remains
  available for a quick, disposable in-memory DB (and is what the
  automated test suite always uses, regardless of `.env`).
- **Production DB: staying on Postgres — see Decisions below. Production
  must run `alembic stamp aba742792ce0` once (the baseline revision only —
  never `stamp head`) to record the schema that already exists via the old
  `create_tables()`-on-boot mechanism as already applied, then
  `alembic upgrade head` to actually apply every migration after that
  baseline (e.g. `182b697135b0`'s `note_metrics_scalars` materialized
  view, which does not exist yet on production and has real DDL to run).
  Stamping `head` directly would mark that migration "applied" without
  ever running it, leaving production's `note_metrics` on its old shape
  while the app queries a materialized view that was never created.**
...
2. **Production DB hosting**: **Resolved — staying on Postgres.** The
   ...
   goal anyway.
3. **Alembic migrations**: **Resolved — added 2026-09-13**, see `alembic/`
   and the Database section above. The triggering "real schema change" was
